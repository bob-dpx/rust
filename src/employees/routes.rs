use create::employees::{Employee,Employees};
...
#[post("/employees")]
async fn creata(employee: web::Json<Employee>)->Result<HttpResponse,CustomError>{
    let employee = Employees::creata(employee.into_inner())?;
    Ok(HttpResponse::Ok().json(employee))
}
...
async fn delete(id: web::Path<i32>)->Result<HttpResponse,CustomError>{
    ...
    Ok(HttpResponse::Ok().json(json!({"delete":delete_employee})))
}

pub fn init_routes(config: &mut web::ServiceConfig){
    config.service(find_all);
    config.service(find);
    config.service(creata);
