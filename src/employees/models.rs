...
pub struct Employees{
 ...
 impl Employees{
    ...
    pub fn create(employee:Employees)->Result<Self,CustomError>{
        ...
        let employee=diesel::insert_into(employee::table).values(employee).get_result(&conn)?;
        Ok(employee)
    }
    pub fn update(id:i32,employee:Employee)->Result<Self,CustomError>{
        let conn=db::connection()?;
        let employee = diesel::update(employee::table).filter(employees::id.eq(id)).set(employee).get_result(&conn)?;
        Ok(employee)
    }
